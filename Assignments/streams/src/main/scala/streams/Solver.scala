...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    val neighbors = b.legalNeighbors.toStream

    neighbors map { case (block, move) =>
      (block, move :: history)
    }
  }
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
                       explored: Set[Block]): Stream[(Block, List[Move])] = neighbors filter { case (block, _) =>
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    val allNeighbors = initial flatMap { case (block, moves) =>
      val neighbors = neighborsWithHistory(block, moves)
      newNeighborsOnly(neighbors, explored)
    }

    initial #::: from(allNeighbors,explored ++ initial.map(x => x._1))
  }
  ...
  /**
    ...
    */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart filter { case (block, _) =>
    done(block)
  }
